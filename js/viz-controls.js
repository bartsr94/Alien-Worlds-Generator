...
// Wind and ocean layer sets — referenced by applyLayer here and imported by main.js.
const WIND_LAYERS  = new Set(['pressureSummer', 'pressureWinter', 'windSpeedSummer', 'windSpeedWinter']);
const OCEAN_LAYERS = new Set(['oceanCurrentSummer', 'oceanCurrentWinter']);
export const isWindLayer  = l => WIND_LAYERS.has(l);
export const isOceanLayer = l => OCEAN_LAYERS.has(l);
...
// Legend rendering — each _legendXxx(el) sets innerHTML and optionally wires DOM events.
function _makeLegendGradient(colorStops, labels) {
    const colors = colorStops.map(([r, g, b]) => `rgb(${Math.round(r*255)},${Math.round(g*255)},${Math.round(b*255)})`);
    const pcts   = colors.map((_, i) => Math.round(i / (colors.length - 1) * 100));
    const grad   = colors.map((c, i) => `${c} ${pcts[i]}%`).join(', ');
...
function _legendTerrain(el) {
    const hasOcean  = state.planetaryParams?.hasLiquidOcean !== false;
    const elevStops = [-0.50, -0.25, -0.05, 0.00, 0.03, 0.15, 0.35, 0.55, 0.80];
    el.innerHTML = _makeLegendGradient(
        elevStops.map(e => elevationToColor(e)),
        [hasOcean ? 'Deep Ocean' : 'Deep Basin', hasOcean ? 'Sea Level' : 'Ground', 'Peak']
    );
}

function _legendKoppen(el) {
    ...
    for (let i = 1; i < KOPPEN_CLASSES.length; i++) {
        ...
        const [r, g, b] = k.color;
        html += `<div class="legend-koppen-item" data-code="${k.code}"><span class="legend-koppen-swatch" style="background:rgb(${Math.round(r*255)},${Math.round(g*255)},${Math.round(b*255)})"></span>${k.code}</div>`;
    }
    html += '<div class="legend-koppen-tooltip" id="koppenTip"></div></div>';
    el.innerHTML = html;
    const tipEl     = document.getElementById('koppenTip');
    const container = el.querySelector('.legend-koppen');
    el.querySelectorAll('.legend-koppen-item').forEach(item => {
        item.addEventListener('mouseenter', () => {
            ...
            if (classId >= 0) { state.hoveredKoppen = classId; updateKoppenHoverHighlight(); updateMapKoppenHoverHighlight(); }
        });
        item.addEventListener('mouseleave', () => {
            tipEl.classList.remove('visible');
            state.hoveredKoppen = -1; updateKoppenHoverHighlight(); updateMapKoppenHoverHighlight();
        });
    });
}

function _legendBiome(el) {
    const stops = [
        { c: [0.82,0.72,0.50], l: 'Desert'  },
        { c: [0.72,0.62,0.30], l: 'Steppe'  },
        { c: [0.42,0.50,0.18], l: 'Savanna' },
        { c: [0.12,0.38,0.10], l: 'Forest'  },
        { c: [0.06,0.22,0.08], l: 'Taiga'   },
        { c: [0.35,0.32,0.22], l: 'Tundra'  },
        { c: [0.78,0.80,0.84], l: 'Ice'     },
    ];
    el.innerHTML = _makeLegendGradient(stops.map(s => s.c), [stops[0].l, stops[3].l, stops[6].l]);
}

function _legendRainShadow(el) {
    el.innerHTML =
        `<div class="legend-gradient" style="background:linear-gradient(to right,rgb(230,51,33) 0%,rgb(140,140,148) 50%,rgb(38,102,243) 100%)"></div>` +
        `<div class="legend-labels"><span>Rain Shadow</span><span>Neutral</span><span>Windward</span></div>`;
}

function _legendHeightmap(el) {
    el.innerHTML =
        `<div class="legend-gradient" style="background:linear-gradient(to right,#000 0%,#404040 25%,#fff 100%)"></div>` +
        `<div class="legend-labels"><span>Ocean Floor</span><span>Sea Level</span><span>Peak</span></div>`;
}

function _legendLandHeightmap(el) {
    el.innerHTML =
...
function _legendHabitability(el) {
    el.innerHTML =
        `<div class="legend-gradient" style="background:linear-gradient(to right,rgb(51,20,20) 0%,rgb(191,102,13) 30%,rgb(191,170,13) 55%,rgb(20,184,56) 100%)"></div>` +
        `<div class="legend-labels"><span>Inhospitable</span><span>Marginal</span><span>Habitable</span></div>`;
}

const _LEGEND_BUILDERS = {
    '':                 _legendTerrain,
    'koppen':           _legendKoppen,
    'biome':            _legendBiome,
    'rainShadowSummer': _legendRainShadow,
    'rainShadowWinter': _legendRainShadow,
    'heightmap':        _legendHeightmap,
    'landheightmap':    _legendLandHeightmap,
    'hydroState':       _legendHydroState,
    'habitability':     _legendHabitability,
};

export function updateLegend(layer) {
    if (!vizLegend) return;
    const builder = _LEGEND_BUILDERS[layer ?? ''];
    if (builder) builder(vizLegend);
    else vizLegend.innerHTML = '';
}
