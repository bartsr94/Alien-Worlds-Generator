...
- **Colony system** — found settlements on planetary tiles from the tile detail panel (requires climate to be computed). Founding costs resources from the body's shared resource pool; cost scales with tile habitability (hostile tiles cost roughly twice as much). Each colony goes through a bootstrap phase (90–180 days depending on habitability) during which it produces at 20% capacity before operating at full rate. Maintenance is drawn from the pool each tick. Colonies collapse after 3 consecutive ticks with unpaid food upkeep. Colonies level up through five tiers (Outpost → Settlement → Colony → City → Megacity) as population grows. A floating **Settlement Panel** in the top-right shows the body's total resource pool, net production rates per resource (color-coded green/red), and per-settlement state (bootstrap progress bar or active prod/maint/net table); clicking a row flies the camera to that tile and opens its tile panel.
...
The **Settlement Panel** (top-right of the screen) shows the body's shared resource pool totals (food, water, metals, fuel), color-coded aggregate net rates (green = surplus, red = deficit), and a row per settlement showing its tier, population, and state (bootstrap progress bar or active prod/maint/net mini-table); starvation warnings appear in red. Clicking a row flies the camera to that tile and opens its tile detail panel. Colonies and their pool are cleared when the planet is rebuilt.
